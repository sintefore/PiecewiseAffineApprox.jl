@testset "Big-M calculations" begin
    ...
    pwl₁ = approx(
        ...
        Convex(),
        Optimized(optimizer = optimizer, planes = 5),
    )
    pwl₁⁺ = approx(
        FunctionEvaluations(Tuple.(x), z),
        Convex(),
    ...
    @testset "Duplicate inputs" begin
        ...
        @test_broken !isnan(
            PWA._linear_big_M(FunctionEvaluations(tuple.(x), z)),
        )
        @test_broken PWA._linear_big_M(
            FunctionEvaluations(tuple.(unique(x)), unique(z)),
        ) ≈ 17.02 atol = 0.01
    end
end
