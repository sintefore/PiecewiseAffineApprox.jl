...
pwl = approx(vals, Convex(), Heuristic(); optimizer=Xpress.Optimizer, nseg=5)
...
pwl = approx(vals, Convex(), Heuristic(); optimizer=Xpress.Optimizer, nseg=7, strict=:below)
...
pwl = approx(vals, Convex(), Heuristic(), optimizer=Xpress.Optimizer, nseg=10)
PWL.plotconv2D(vals, pwl)
...
f(x) = max(x[1]^2,x[2]^2)
vals = PWL.sample_uniform(f, [(-1,1), (-1,1)], 10)
pwl = approx(vals, Convex(), Heuristic(), optimizer=Xpress.Optimizer, nseg=8, pen=:max)
PWL.plotconv2D(vals, pwl)
...
pwl = approx(vals, Convex(), Heuristic(), optimizer=Xpress.Optimizer, nseg=8, pen=:rms)