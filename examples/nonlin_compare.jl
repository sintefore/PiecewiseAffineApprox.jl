...
hypatia = optimizer_with_attributes(Hypatia.Optimizer, 
        MOI.Silent() => true,
    )
pajarito = optimizer_with_attributes(Pajarito.Optimizer,
        "time_limit" => 60, 
        "oa_solver" => oa_solver, 
...
for sz in StepRange(5, 5, 20)
    ...
    push!(results, (size=sz, formulation="pwl", solver="highs", time=time_opt.time, obj_val=obj_val))
    ...
    push!(results, (size=sz, formulation="nonlin", solver="highs", time=time_opt.time, obj_val=obj_val))
