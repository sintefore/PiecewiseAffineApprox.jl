...
function convex_pwlinear(
    ...
    if isnothing(z)
        z = JuMP.@variable(
            m,
            lower_bound = minimum(pwl.z),
            upper_bound = maximum(pwl.z),
...
function convex_pwlinear(
    ...
    return convex_pwlinear(
        m,
        x,
        convex_linearization(xd, zd, optimizer; kwargs...),
        z = z,
    )
end
...
function convex_pwlinear(
    ...
    return convex_pwlinear(
        m,
        x,
        convex_linearization(f, xmin, xmax, optimizer; kwargs...),
        z = z,
    )
end

function concave_pwlinear(
    m::JuMP.Model,
    x::VarOrAff,
    pwl::ConcavePWLFunction;
    z = nothing,
)
    initPWL!(m)
    counter = m.ext[:PWL].counter + 1
...
function concave_pwlinear(
    ...
    return concave_pwlinear(
        m,
        x,
        convex_linearization(xd, fd, optimizer, kwargs...),
        z = z,
    )
end
...
function convex_pwlinear(
    ...
    function minz(pwl)
        return minimum(p.β for p ∈ pwl.planes)
    end
    function maxz(pwl)
        return maximum(p.β for p ∈ pwl.planes)
    end
...
function concave_pwlinear(
    ...
    return concave_pwlinear(
        m,
        x,
        concave_linearization(xd, fd, optimizer, kwargs...),
        z = z,
    )
end
...
function concave_pwlinear(
    ...
    return concave_pwlinear(
        m,
        x,
        concave_linearization(f, xmin, xmax, optimizer, kwargs...),
        z = z,
    )
end
...
function concave_pwlinear(
    ...
    return concave_pwlinear(
        m,
        x,
        concave_linearization(xd, zd, optimizer; kwargs...),
        z = z,
    )
end
